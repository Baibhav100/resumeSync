...
const AdminRegister = () => {
  ...
  useEffect(() => {
    document.title = 'Admin Register | AutoResume.ai';
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { name, email, password, conpassword, adminKey } = formData;

    if (!name.trim() || !email.trim() || !password.trim() || !conpassword.trim() || !adminKey.trim()) {
      await Swal.fire({
        icon: 'warning',
    ...
    setLoading(true);
    try {
      await url.post('/admin/register', { name, email, password, adminKey });
      await Swal.fire({
        icon: 'success',
        title: 'Admin account created',
        text: 'You can now sign in using admin credentials.',
        timer: 1800,
      ...
      await Swal.fire({
        ...
        text: error.response?.data?.message || 'Unable to create admin account',
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="hidden lg:flex w-1/2 flex-col justify-between p-12 bg-slate-900 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg mb-20">
          <h1 className="text-5xl font-extrabold text-white leading-tight mb-6 font-heading">
            Admin sign-up with the same polished experience as our users.
          </h1>
          <p className="text-slate-300 text-lg leading-relaxed font-light">
            Create your secure admin account using your private key, then manage users and resume analytics from a dedicated dashboard.
          </p>
          <div className="mt-12 flex items-center gap-4">
            ...
            <p className="text-sm text-slate-400 font-medium">Admin access with secure onboarding.</p>
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 relative">
        <div className="w-full max-w-md">
          ...
          <div className="mb-10 text-center lg:text-left">
            <h2 className="text-3xl font-bold text-slate-900 mb-2 font-heading tracking-tight">Register as admin</h2>
            <p className="text-slate-500 text-sm sm:text-base">Create your admin account with a secure key.</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? <span className="loading loading-spinner loading-sm"></span> : 'Create admin account'}
            </button>
            <div className="relative py-6 flex items-center">
              <div className="flex-grow border-t border-slate-200"></div>
              <span className="flex-shrink mx-4 text-slate-400 text-xs font-semibold uppercase tracking-wider">Already have admin access?</span>
              <div className="flex-grow border-t border-slate-200"></div>
            </div>
            <Link to="/admin/login" className="w-full flex justify-center py-3.5 px-4 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-xl font-semibold transition-colors active:scale-[0.98]">
              Sign in as admin
            </Link>
          </form>
        </div>
